  ...
  <section class="members-modal">
    <div class="members-modal-container">
      ...
      <div class="members-list flex column" v-if="boardMembers.length">
        <div
          class="member flex space-between align-center"
          v-for="memberId in boardMembers"
          :key="getUsername(memberId)"
          @click="toggleMemberOnTask(memberId)"
        >
  ...
  methods: {
    ...
    toggleMemberOnTask(memberId) {
      ...
      if (this.isUserAssignedToTask(memberId)) {
        ...
        this.$store.dispatch("addMemberToTask", {
          userId: memberId._id,
          task: this.task
        });
